...
| WHEN      | ROLE                                   | WHERE                    |
| :-------- | :------------------------------------- | :----------------------- |
| 2022-now  | **Software engineer**                  | Betacom srl              |
| 2018-2022 | **IT specialist**                      | Bonprix srl              |
| 2018      | **Web developer**               | Orange Pix srl           |
| 2017      | **Web developer**               | 3w srl                   |
...
- **Software Engineer at Betacom**:
  
  I mantain with my team a web application based on microservice architecture. The frontend is built with ReactJs, wich is the part of the application I am responsible as a frontend developer specialist. The backend services are written in java, and I take care of their maintenance sporadically.  
  I also take care, together with the rest of the team, of the maintenance and configuration of the Windows servers on which the various microservices run via Apache Web Server and Tomcat.  
  Furthermore I started others projects of web applications used internally, like a creation of a UI library built with Web Components and served as an npm package.
...
- **IT specialist at Bonprix**:
  
  - I wrote a software in Java for the generation of stock feed, with communication to db through sql queries
...
`reactjs` `javascript` `typescript` `java` `non-relational database` `relational database` `sql` `web components` `angular` `vite` `git` `github` `node-js` `jira` `css` `cookie` `postman` `scrum` `teamwork` `problem solving` `analysis` `rest api` `performance patterns` `ux`
